...
def plot_sgi_distributions(
    ...
    for i, model_name in enumerate(model_names):
        ...
        sns.kdeplot(grounded, ax=ax, color=colors[i], linestyle='-', 
                   alpha=0.7, label=f'{model_name} (grounded)')
...
def plot_scatter_comparison(
    ...
    ax.scatter(df[~grounded_mask][col1], df[~grounded_mask][col2],
               alpha=0.5, c='#e74c3c', label='Hallucinated', s=30)
    ax.scatter(df[grounded_mask][col1], df[grounded_mask][col2],
               alpha=0.5, c='#2ecc71', label='Grounded', s=30)
...
def create_summary_figure(
    ...
    # 1. Effect sizes
    plot_effect_sizes(effect_sizes, axes[0, 0], metric='cohens_d',
                     title="Effect Size (Cohen's d) Across Models")
    
    # 2. AUROC
    plot_effect_sizes(effect_sizes, axes[0, 1], metric='auroc',
                     title="Classification Performance (AUROC)")
    
    # 3. SGI distributions
    plot_sgi_distributions(df, model_names, axes[1, 0],
                          title="SGI Distributions: Grounded (solid) vs Hallucinated (dashed)")
    
    # 4. Spearman correlation
    plot_correlation_heatmap(spearman_matrix, axes[1, 1],
                            title="Ranking Agreement (Spearman ρ)")
    
    fig.suptitle("Cross-Model Validation: Is SGI Fundamental?",
